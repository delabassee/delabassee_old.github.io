...
[Cargo](http://java.net/projects/cargotracker/sources/svn/content/trunk/src/main/java/net/java/cargotracker/domain/model/cargo/Cargo.java) has both a clear identity and a life-cycle with state transitions that we care about, so it's an entity. It is obviously a key concept in the domain. Many cargo instances will exist in the system simultaneously. The different instances may have the same origin and destination, they may even contain the same kind of things, but it is important for us to be able to track individual cargo instances. In our case the cargo's identity is its tracking id. The tracking id is assigned upon creation and is never changed. This is the handle used to track the cargo's progress (e.g. from the tracking website).

The cargo's delivery state will change during it's lifetime. It's transport status will start out as NOT_RECEIVED, i.e. booked but not yet handed over to the shipping company at the port, and in the normal case ends its life as CLAIMED (note that this is a property of the cargo's [Delivery](http://java.net/projects/cargotracker/sources/svn/content/trunk/src/main/java/net/java/cargotracker/domain/model/cargo/Delivery.java)), tracking the current state the cargo. During a cargo's lifetime it may be assigned new destinations, its itinerary may be changed many times and it's delivery will be recalculated as new [HandlingEvents](http://java.net/projects/cargotracker/sources/svn/content/trunk/src/main/java/net/java/cargotracker/domain/model/handling/HandlingEvent.java) are received.

![ ](/ct/pic/cargo.png)

A [Voyage](http://java.net/projects/cargotracker/sources/svn/content/trunk/src/main/java/net/java/cargotracker/domain/model/voyage/Voyage.java) is a vessel's trip from origin to destination, typically made up of several segments ([CarrierMovements](http://java.net/projects/cargotracker/sources/svn/content/trunk/src/main/java/net/java/cargotracker/domain/model/voyage/CarrierMovement.java)). In the application a [Voyage](http://java.net/projects/cargotracker/sources/svn/content/trunk/src/main/java/net/java/cargotracker/domain/model/voyage/Voyage.java) consists of a [Schedule](http://java.net/projects/cargotracker/sources/svn/content/trunk/src/main/java/net/java/cargotracker/domain/model/voyage/Schedule.java) with the different [CarrierMovements](http://java.net/projects/cargotracker/sources/svn/content/trunk/src/main/java/net/java/cargotracker/domain/model/voyage/CarrierMovement.java) in it and has a very clear notion of identity, [VoyageNumber](http://java.net/projects/cargotracker/sources/svn/content/trunk/src/main/java/net/java/cargotracker/domain/model/voyage/VoyageNumber.java). This id could be something like a flight number for air shipments or a vessel voyage number for a ship, it is not the name or the identification of the actual vessel.
...
A [Leg](http://java.net/projects/cargotracker/sources/svn/content/trunk/src/main/java/net/java/cargotracker/domain/model/cargo/Leg.java) consists of a starting point and an ending point (to [Location](http://java.net/projects/cargotracker/sources/svn/content/trunk/src/main/java/net/java/cargotracker/domain/model/location/Location.java) and from [Location](http://java.net/projects/cargotracker/sources/svn/content/trunk/src/main/java/net/java/cargotracker/domain/model/location/Location.java)), and a reference to a voyage. A leg has no sense of identity; two legs with the same from location, end location and voyage are in our model completely interchangeable. As a result, we implement leg as an immutable Value Object.

An [Itinerary](http://java.net/projects/cargotracker/sources/svn/content/trunk/src/main/java/net/java/cargotracker/domain/model/cargo/Itinerary.java) consists of a list of legs, with the load location of the first leg in the list as the starting point of the itinerary and the unload location of the last leg as the final destination. The same reasoning applies to itineraries as to legs, they do not have identity and are implemented as Value Objects. Now, a cargo can certainly have its itinerary updated. One way to accomplish this would be to keep the original itinerary instance and update the legs in the itinerary's list, in this case the itinerary must be mutable and has to be implemented as an entity. With the itinerary as a Value Object, as in the case of our application model and implementation, updating it is a simple operation of acquiring a complete new itinerary from the [RoutingService](http://java.net/projects/cargotracker/sources/svn/content/trunk/src/main/java/net/java/cargotracker/domain/service/RoutingService.java) and replacing the old one. Implementation of a cargo's itinerary management is much simplified by having the itinerary as a Value Object.
...
A good example of that is the [RoutingService](http://java.net/projects/cargotracker/sources/svn/content/trunk/src/main/java/net/java/cargotracker/domain/service/RoutingService.java), which provides access to the routing system and is used to find possible routes for a given specification. The implementation, [ExternalRoutingService](http://java.net/projects/cargotracker/sources/svn/content/trunk/src/main/java/net/java/cargotracker/infrastructure/routing/ExternalRoutingService.java), communicates with another system and translates to/from an external API/data model in the infrastructure layer.
...
Application services represent the high level business operations for the system and constitute the application layer. They provide a high-level abstraction for clients to use when interacting with the domain. The [`net.java.cargotracker.application`](https://java.net/projects/cargotracker/sources/svn/show/trunk/src/main/java/net/java/cargotracker/applicationn) package container all the services for the application such as [BookingService](http://java.net/projects/cargotracker/sources/svn/content/trunk/src/main/java/net/java/cargotracker/application/BookingService.java) and [HandlingEventService](http://java.net/projects/cargotracker/sources/svn/content/trunk/src/main/java/net/java/cargotracker/application/HandlingEventService.java). The application services are a natural place to apply concerns such as pooling, transactions and security. This is why application services are typically implemented using EJB or transactional CDI beans.

![ ](/ct/pic/booking_service.png)

In some situations, e.g. when dealing with graphs of lazy-loaded domain objects or when passing services' return values over network boundaries, the services are wrapped in facades. The facades handle ORM session management issues and/or convert the domain objects to more portable Data Transfer Objects that can be tailored to specific use cases. In that case, we consider the DTO-serializing facade part of the interfaces layer. See [BookingServiceFacade](http://java.net/projects/cargotracker/sources/svn/content/trunk/src/main/java/net/java/cargotracker/interfaces/booking/facade/BookingServiceFacade.java) for an example
